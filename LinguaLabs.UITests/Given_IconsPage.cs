...
{    [Test]
    public async Task When_NavigateToIcons_ShouldNotCrash()
    {
        ...
        try
        {
            ...
            if (App.Query(iconsButton).Length != 0)
            {
                App.Tap(iconsButton);
            ...
            else if (App.Query(showIconsButton).Length != 0)
            {
            ...
            else if (App.Query(iconsText).Length != 0)
            {
                App.Tap(iconsText);
            ...
            else
            {
                ...
                if (App.Query(anyIconElement).Length != 0)
                {
                    TakeScreenshot("Icons_Element_Found");
                }
                else if (App.Query(iconsFlyout).Length != 0)
                {
                    TakeScreenshot("IconsFlyout_Found");
                }
                else if (App.Query(iconsPage).Length != 0)
                {
